...
function sortProducts(type) {
  const grid = document.querySelector('.products-section .products-grid'); // TRENDY
  sortGrid(grid, type);
}

function sortSideProducts(type) {
  const grid = document.querySelector('.small-grid'); // TYČINKY
  sortGrid(grid, type);
}

function sortGrid(grid, type) {
  if (!grid) return;
